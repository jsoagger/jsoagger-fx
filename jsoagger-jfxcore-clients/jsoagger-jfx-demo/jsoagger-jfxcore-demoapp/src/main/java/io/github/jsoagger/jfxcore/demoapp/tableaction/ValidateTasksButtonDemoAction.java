 ...
 *
 */
public class ValidateTasksButtonDemoAction extends AbstractAction implements IAction {
   ...
   * Default Constructor
   */
  public ValidateTasksButtonDemoAction() {
    super();
  }
  ...
  public Object onOk(Object o) {
    for(OperationData d: controller.getTableStructure().getItems()) {
      ...
      if("waiting for acceptance".equalsIgnoreCase(status)) {
        d.getAttributes().put("status","in repair");
      }
    }
    ...
    NodeHelper.showHeaderMessage(controller, "Task(s) successfully validated", "gmi-cloud-done:32");
    return null;
  }
