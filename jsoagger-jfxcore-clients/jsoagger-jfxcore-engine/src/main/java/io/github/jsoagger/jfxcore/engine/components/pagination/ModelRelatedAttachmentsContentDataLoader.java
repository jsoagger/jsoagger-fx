 ...
 * @date 20 janv. 2018
 */
public class ModelRelatedAttachmentsContentDataLoader implements IPaginatedDataProvider {
  ...
  public void navigate(IJSoaggerController controller, IOperationResult currentPage, Direction direction, Consumer<IOperationResult> consumer) {
    ...
    query.addProperty("role", "attachments");

    final IOperation getContentInfoOperation = (IOperation) Services.getBean("GetContentInfoOperation");
    getContentInfoOperation.doOperation(query, res -> consumer.accept(res));
  }
}
