 ...
 * @date 15 févr. 2018
 */
public class GenericModelAttributetoTablePresenter extends CellPresenterImpl implements IModelAttributePresenter {
  ...
  public Node present(IJSoaggerController controller, VLViewComponentXML configuration) {
    ...
    final IOperationResult model = (IOperationResult) controller.getModel();
    if (model != null) {
      final OperationData data = (OperationData) model.rootData();
      final Object datavalue = ReflectionUIUtils.invokeGetterOn(data, attributePath);
  ...
  public Node present(IJSoaggerController controller, VLViewComponentXML configuration, Object forModel) {
    final String attributePath = configuration.getPropertyValue("attributePath");
    final Object datavalue = ReflectionUIUtils.invokeGetterOn(forModel, attributePath);
    initEnumeratedTypeTranslater(configuration);
