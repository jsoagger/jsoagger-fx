 ...
 * @date 29 janv. 2018
 */
public class DemoModelFlowThumbPresenter extends CellPresenterImpl implements ModelIconPresenter {
   ...
   * Constructor
   */
  public DemoModelFlowThumbPresenter() {
    super();
  ...
  public Node provideIcon(IJSoaggerController controller, VLViewComponentXML configuration, Object forModel) {
    ...
    if (StringUtils.isNotBlank(imagesLocation)) {
      ...
      displayThumb("/defaultimages/" + data.getAttributes().get("id") + ".png");
    }
  ...
  protected void displayEmptyThumb() {
    ...
    final Image img = new Image(NO_THUMB, getWidth() > 0 ? getWidth() : 120, getHeight() > 0 ? getHeight() : 120, true, true);
    ...
    if (CACHE.containsKey(path)) {
      ...
      if(path.startsWith("/default")) {
        ...
        try (InputStream is = new FileInputStream(new File(path))) {
          if (is != null) {
            final Image img = new Image(is, 140, 120, true, true);
            final ImageView imgView = new ImageView(img);
            pane.getChildren().add(imgView);
            CACHE.put(path, pane);
          }
