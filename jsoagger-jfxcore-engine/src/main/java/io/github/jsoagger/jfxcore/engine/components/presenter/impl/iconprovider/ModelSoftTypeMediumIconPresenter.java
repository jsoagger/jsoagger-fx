 ...
 * @date 29 janv. 2018
 */
public class ModelSoftTypeMediumIconPresenter extends CellPresenterImpl implements ModelIconPresenter {
  ...
  public Node provideIcon(IJSoaggerController controller, VLViewComponentXML configuration) {
    return ModelIconUtils.provideSmallIcon((AbstractViewController)controller);
  }
  ...
  public Node provideIcon(IJSoaggerController controller, VLViewComponentXML configuration, Object forModel) {
    ...
    String typepath = (String) data.getBusinessType().get("logicalPath");
    if (StringUtils.isEmpty(typepath)) {
      typepath = (String) data.getBusinessType().get("internalType");
    }
    return ModelIconUtils.provideMediumIcon(typepath);
  }
