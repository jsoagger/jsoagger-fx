...
public class MenuRow extends VBox {
  ...
  public MenuRow(VLViewRootMenuRowXML menuItemConfig, Menu menu, MenuStructure menuStructure, AbstractViewController controller) {
    ...
    getChildren().add(row);
    if(!leaf.get()) {
    	getChildren().add(rowsubmenupane);
    }
   ...
   * @param subMenu
   */
  public void setSubmenu(SubMenu subm, boolean onRow) {
    subMenu = subm;
    ...
    row.getChildren().addAll(NodeHelper.horizontalSpacer(), pane);

    row.addEventFilter(MouseEvent.MOUSE_CLICKED, a -> collapseExpandSubmenu());
    ...
    row.pseudoClassStateChanged(PseudoClass.getPseudoClass("notleaf"), true);
    ...
    if(onRow) {
    	...
    	rowsubmenupane.getStyleClass().add("ep-submenu-row-1");
    }
    else {
    	//rowsubmenupane.getChildren().add(subMenu);
    	//rowsubmenupane.getStyleClass().add("ep-submenu-row-1");
    }
  ...
  private class PMDoActionTask extends Task<Void>{
    ...
    protected Void call() throws Exception {
      ...
      // load a child view
        ...
        NodeHelper.location(menuItemConfig, controller);
      }
