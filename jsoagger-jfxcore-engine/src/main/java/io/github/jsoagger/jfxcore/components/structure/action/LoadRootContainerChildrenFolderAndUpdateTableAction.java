...
public class LoadRootContainerChildrenFolderAndUpdateTableAction extends AbstractAction {
   ...
   * Constructor
   */
  public LoadRootContainerChildrenFolderAndUpdateTableAction() {
    super();
  }
  ...
  public void execute(IActionRequest actionRequest, Optional<IActionResult> previousActionResult) {
    ...
    if (io.github.jsoagger.core.utils.StringUtils.isNotBlank(operation)) {
      ...
      op.doOperation(query, resp -> {
        ...
        // force select first elemen
        // may be some adjacent controller need this data
        MultipleResult mr = (MultipleResult) resp;
        if(mr.getData()!= null && mr.getData().size() > 0) {
          controller.selectedElementProperty().set(mr.getData().get(0));
        }
        else {
          controller.selectedElementProperty().set(null);
          ts.setNoContent();
        }
      }, ex -> {
        controller.selectedElementProperty().set(null);
