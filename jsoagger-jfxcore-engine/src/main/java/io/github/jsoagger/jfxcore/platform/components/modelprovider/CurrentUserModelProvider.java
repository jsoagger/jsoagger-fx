 ...
 *
 */
public class CurrentUserModelProvider implements IModelProvider {

  private IOperation getUserDetailsOperation;// needs GetUserDetailsOperation


  public CurrentUserModelProvider() {}
  ...
  public IOperationResult loadModel(IJSoaggerController controller, String componentId) {
    ...
    op.doOperation(new JsonObject(), res ->{
      SingleResult sr = (SingleResult) res;

      final String login = (String) sr.getData().getAttributes().get("nickName");

      final JsonObject query = new JsonObject();
      query.addProperty("login", login);

      getUserDetailsOperation.doOperation(query, resu -> {
        controller.setModel(resu);
        ((AbstractViewController) controller).setModel(resu);
      });
    });
