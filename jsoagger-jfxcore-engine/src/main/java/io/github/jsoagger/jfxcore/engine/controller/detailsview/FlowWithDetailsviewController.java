 ...
 * @date 4 févr. 2018
 */
public class FlowWithDetailsviewController extends StandardViewController {

  StandardViewController leftSection;
   ...
   * Default Constructor
   */
  public FlowWithDetailsviewController() {
    super();
  }
  ...
  protected void process() {
    ...
    if (StringUtils.isNotBlank(leftSectionView)) {
      leftSection = StandardViewUtils.forId(getRootStructure(), structureContentController, leftSectionView);
    ...
    // IF YOU WANT TO RELOAD DETAILS SET MODEL TO NULL IN STRUCTURE CONTENT
    // AND SET IT TO A NEW VALUE AGAIN AND IT WILL BE RELOADED
    detailsView = getRootComponent().getPropertyValue("rightSectionView");
    getStructureContentController().formModelDataProperty().addListener((ChangeListener<OperationData>) (observable, oldValue, newValue) -> {
      if (newValue != null) {
        loadDetailsTask();
      }
    });
