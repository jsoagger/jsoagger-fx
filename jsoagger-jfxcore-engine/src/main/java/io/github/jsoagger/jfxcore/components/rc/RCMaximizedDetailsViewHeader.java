...
public class RCMaximizedDetailsViewHeader implements IBuildable, IToolbarHolder, IDetailsViewHeader {
  ...
  public void buildFrom(IJSoaggerController controller, VLViewComponentXML configuration) {
    ...
    workStatusContainer.managedProperty().bind(Bindings.isNotEmpty(workStatusContainer.getChildren()));
    if (presenter.getWorkStatusPresenter() != null) {
      final Node ws = presenter.getWorkStatusPresenter().present(controller, configuration);
      // ainer.getChildren().add(ws);
    }
    ...
    // build actions
    // actions are builded on class maximizedDetailsView
  ...
  public void hideDetailsIdentity() {
    identityContainer.getChildren().clear();
  ...
  public void showIdentity() {
    final Node identity = getHeaderIdentity();
    identityContainer.getChildren().add(identity);
