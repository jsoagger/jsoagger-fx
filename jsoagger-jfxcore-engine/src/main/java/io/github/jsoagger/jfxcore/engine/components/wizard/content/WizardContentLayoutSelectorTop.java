 ...
 * @date 2019
 */
public class WizardContentLayoutSelectorTop extends WizardContentLayoutSelector implements IWizardContentLayout {
   ...
   * Constructor
   */
  public WizardContentLayoutSelectorTop() {
    super();
    getStyleClass().add("wizard-content-layout-top-wrapper");
    header.getStyleClass().add("wizard-content-layout-top-listViewPaneHeader");
    stepTopWrapper.getChildren().add(header);
  }
  ...
  public void buildFrom(IJSoaggerController controller, VLViewComponentXML configuration) {
    ...
    layout.getChildren().add(stepTopWrapper);
    layout.getChildren().add(stepContentWrapper);
    NodeHelper.setHVGrow(stepContentWrapper, layout);
  ...
  public void addStep(IWizardStep step) {
    IWizardStepper stepper = (IWizardStepper) Services.getBean("WizardHorizontalStepper");
    stepper.setStep(step);
    steppers.add(stepper);
    header.getChildren().add(stepper.getDisplay());
